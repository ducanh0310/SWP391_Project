...
<html lang="en">
    ...
    <body>
        <div id="AddServiceSuccessNotification" class="position-fixed top-0 end-0 p-3">
            <div id="AddServiceSuccessAlert" class="alert alert-success alert-dismissible fade show mb-0" role="alert">
                ${sessionScope.addServicesSuccess}
                <button type="button" class="btn-close" id="closeAddServiceSuccessButton" aria-label="Close"></button>
                <div class="progress mt-2" style="height: 4px;">
                    <div id="AddServiceSuccessProgressBar" class="progress-bar progress-bar-animated bg-success" role="progressbar" style="width: 0%;" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
            </div>
        </div>
        <!-- Popup Modal -->
                            ...
                            <div class="container">
                                <div class="header_wrap">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <a href="addservice" class="btn btn-primary btn-sm">Add Service</a>
                                    </div>
                                            ...
                                            <table class="table table-striped table-class" id= "table-id">
                                                ...
                                                <tbody>
                                                    <c:forEach items="${services}" var="service">
                                                        <tr id="service${service.procedure_id}">
                                                            <td> ${service.procedure_name}
                                                            </td>
                                                            <td>${'$'}${service.price}</td>
                                                            <td>
                                                                <span class="badge badge-lg badge-dot">
                                                                    <c:choose>
                                                                        <c:when test="${service.type == 'a'}">
                                                                            <span class="badge badge-lg badge-dot">
                                                                                <i class="bg-success"></i>Active
                                                                            </span>
                                                                        </c:when>
                                                                        <c:when test="${service.type == 'i'}">
                                                                            <span class="badge badge-lg badge-dot">
                                                                                <i class="bg-danger"></i>Inactive
                                                                            </span>
                                                                        </c:when>
                                                                        <c:otherwise>${service.type}</c:otherwise>
                                                                    </c:choose>

                                                                </span>
                                                            </td>
                                                            <td class="text-end">
                                                                <a href="ViewServiceDetail?id=${service.procedure_id}" class="btn btn-sm btn-neutral">View</a>
                                                                <form action="deleteService" method="POST" style="display: inline-block;">
                                                                    <input type="hidden" name="serviceID" value="${service.procedure_id}">
                                                                    <button type="submit" class="btn btn-sm btn-square btn-neutral text-danger-hover" onclick="confirmDelete(${service.procedure_id})">
                                                                        <i class="bi bi-trash"></i>
...
<script src="js/suggestions.js"></script>
        ...
        <script src="js/main.js"></script> 
        <script>
                                                                        function showAddServiceSuccessNotification() {
                                                                            $('#AddServiceSuccessNotification').show();
                                                                            let progressBar = $('#AddServiceSuccessProgressBar');
                                                                            let width = 0;
                                                                            let interval = setInterval(function () {
                                                                                ...
                                                                                if (width === 100) {
                                                                                    ...
                                                                                    $('#AddServiceSuccessNotification').fadeOut();
                                                                                }
                                                                            }, 50);
                                                                        }
                                                                        // Close notification button handler
                                                                        $('#closeAddServiceSuccessButton').click(function () {
                                                                            $('#AddServiceSuccessNotification').hide();
                                                                        });

                                                                        $(document).ready(function () {
                                                                            let addServicesSuccess = '${sessionScope.addServicesSuccess}';
